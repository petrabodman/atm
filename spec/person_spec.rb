...
describe Person do
  subject { described_class.new(name: 'Petra') }

  it 'is expeced to have a :name on initialize' do
    expect(subject.name).not_to be nil
  end

  it 'is expeced to raise error if no name is set' do
    expect { described_class.new }.to raise_error
    'A name is required'
  end

  it 'is expected to have a :cash attribute with
  value of 0 on initialize' do
  ...
  describe 'can not manage funds if no account been created' do
    it 'can\'t deposit funds' do
      expect { subject.deposit(100) }.to raise_error(RuntimeError, 'An account is required')
    end
  end
end
